...
const ViewUnregistered = () => {
  ...
  useEffect(() => {
    const fetchPaidRegistrations = async () => {
      try {
        const q = query(collection(db, "registered"), where("paymentStatus", "==", false));
  ...
  return (
    <div className="w-full p-5 md:p-15 flex justify-center items-center">
        ...
        <h2 className="text-xl font-bold mb-4">Unapproved Registrations</h2>
        {registrations.length > 0 ? (
            ...
            <p>No unpaid registrations found.</p>
        )}
        </div>
    </div>
  );
